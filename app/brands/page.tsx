...
export default async function BrandsIndexPage() {
  ...
  return (
    <section className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-10 pt-10 sm:pt-16 pb-20">
      <ScrollReveal>
        <p className="text-xs uppercase tracking-wide2 text-ink-muted">Shop by</p>
      ...
      {brands.length === 0 ? (
        <div className="mt-12 border border-dashed border-sand-200 rounded-xl py-20 text-center">
          <p className="font-display text-2xl">No brands yet.</p>
          <p className="mt-2 text-sm text-ink-muted">
            Add products in the Studio at <code>/studio</code> and they&apos;ll be grouped here automatically.
          </p>
        </div>
      ) : (
        <ul className="mt-12 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-3">
          {brands.map((b, i) => (
            <ScrollReveal key={b.slug} delay={(i % 8) * 40}>
              <li>
                <Link
                  href={`/brands/${b.slug}`}
                  className="group flex items-baseline justify-between gap-3 py-3 border-b border-sand-200 press"
                >
